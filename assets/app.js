...
async function setOnboardingDone(){
  const k = `CREATORBOOK_ONBOARD_${APP.session.user.id}`;
...
async function chooseRole(role){
  ...
  // role: 'client' (artist) or 'creator'
  if(role === "creator" && APP.me?.role !== "creator"){
    const up = await APP.sb.from("profiles").update({ role:"creator", approved:false }).eq("id", APP.session.user.id).select("*").single();
    if(up.error) throw up.error;
    APP.me = up.data;
  } else if(role === "client" && APP.me?.role !== "client"){
    const up = await APP.sb.from("profiles").update({ role:"client", approved:false }).eq("id", APP.session.user.id).select("*").single();
    if(up.error) throw up.error;
    APP.me = up.data;
  }
  await setOnboardingDone();
  setAuthedUI();
  // First-time onboarding: ask whether you're booking or creating.
  const { route: r0 } = qs();
  if(r0 !== "welcome" && !isOnboardingDone()){
    location.hash = "#welcome";
    return;
  }

  if(role === "creator") location.hash = "#creator-tools";
  else location.hash = "#home";
  toast(role === "creator" ? "Creator mode (pending approval)" : "Artist mode");
}
async function renderWelcome(){
  showView("viewWelcome");
  const email = APP.session?.user?.email || "";
  const el = $("#welcomeEmail"); if(el) el.textContent = email;
  const bA = $("#btnChooseArtist");
  const bC = $("#btnChooseCreator");
  if(bA) bA.onclick = ()=>chooseRole("client");
  if(bC) bC.onclick = ()=>chooseRole("creator");
}
...
async function route(){
  ...
  const { route, id } = qs();
  if(route === "welcome") await renderWelcome();
