...
function authShow(step){
  const a = byId("authStepRole");
  const b = byId("authStepEmail");
  const c = byId("authStepSent");
  if(a) a.style.display = (step === "role") ? "block" : "none";
  if(b) b.style.display = (step === "email") ? "block" : "none";
  if(c) c.style.display = (step === "sent") ? "block" : "none";
}
...
async function init(){
  ...
  on('btnSignIn','click',()=>{ openModal('authModal'); authShow('role'); });
  on('authClose','click',()=>closeModal('authModal'));
  on('btnBackRole','click',()=>authShow('role'));

  // auth role picks (modal)
  const setRolePick = (r)=>{ APP.rolePicked = r; authShow('email'); };
  on('authPickArtist','click',()=>setRolePick('client'));
  on('authPickCreator','click',()=>setRolePick('creator'));
  on('btnSendLink','click', signInMagicLink);
  // legacy gate buttons (in-page role gate)
  on('pickArtist','click',()=>setRolePick('client'));
  on('pickCreator','click',()=>setRolePick('creator'));
