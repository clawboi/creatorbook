...
function syncWho(){
  ...
  whoPill.style.display='inline-flex';
...
function wireUI(){
  ...
  $('#btnSignOut').onclick = async ()=>{ await APP.sb.auth.signOut(); toast('Signed out'); };

  // Auth role selection
  const goEmail = (role)=>{ localStorage.setItem(ROLE_KEY, role); showAuthStep('email'); };
  $('#authPickArtist').onclick = ()=>goEmail('client');
  $('#authPickCreator').onclick = ()=>goEmail('creator');
  ...
  $('#btnSendLink').onclick = async ()=>{
    ...
    if(error) toast(error.message); else { showAuthStep('sent'); toast('Magic link sent'); }
  };

  // Role gate after login
  $('#pickArtist').onclick = async ()=>{ await setRole('client'); location.hash = '#artist'; };
  $('#pickCreator').onclick = async ()=>{ await setRole('creator'); location.hash = '#creator'; };

  // Home
  $('#homeRefresh').onclick = ()=>renderHomeFeed();

  // Artist
  $('#btnCreateProject').onclick = ()=>{ location.hash = '#create-project'; };
  $('#btnDemoCredits').onclick = ()=>addDemoCredits();
  $('#btnBuyCredits').onclick = ()=>toast('Stripe comes next. Demo credits for now.');

  // Create project
  $('#btnFindCreators').onclick = ()=>renderCreatorResults();
  $('#btnCancelRequest').onclick = ()=>{ APP.chosen=null; $('#requestBox').style.display='none'; };
  $('#btnSendRequest').onclick = ()=>sendBookingRequest();
