...
async function signInWithPassword(){
  ...
  try{
    const { error } = await APP.sb.auth.signInWithPassword({ email: ident, password });
    if(error) return toast(normalizeAuthError(error));
...
async function signUpWithPassword(){
  ...
  try{
    ...
    // 2) Force a sign-in to guarantee we have a session right now.
    // Even with "Confirm email" OFF, some flows can return a null session.
    if(!data?.session){
      const r2 = await APP.sb.auth.signInWithPassword({ email, password });
      if(r2.error) return toast(normalizeAuthError(r2.error));
    }
