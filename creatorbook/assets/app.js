...
function authRedirectTo(){
  ...
  const want = "/creatorbook/creatorbook/";
...
async function completeOnboarding(role){
  ...
  const errBox = byId('onboardErr');
  if(errBox){ errBox.style.display = 'none'; errBox.textContent = ''; }

  // disable buttons so it feels responsive
  setBtnBusy('btnPickArtist', true, 'Saving…');
  setBtnBusy('btnPickCreator', true, 'Saving…');
  ...
  try{
    const patch = {
      ...
      onboarded: true,
      // keep defaults safe if trigger didn't run
      display_name: APP.me?.display_name || (APP.user.email ? APP.user.email.split('@')[0] : 'User'),
      city: APP.me?.city || 'Los Angeles',
      approved: APP.me?.approved ?? false,
    };

    // Use UPSERT so onboarding works even if the profile row was never created.
    ...
    if(r.error){
      const msg = r.error.message || 'Could not save your role. Please try again.';
    ...
    setBtnBusy('btnPickArtist', false, 'Artist');
    setBtnBusy('btnPickCreator', false, 'Creator');
  }
}
