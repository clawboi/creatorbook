...
function setAuthMode(mode){
  ...
  // Role chooser only for signup
  const rw = byId("authRoleWrap");
  if(rw) rw.style.display = (APP.authMode==="signup") ? "" : "none";
...
function normalizeAuthError(err){
  ...
  if(low.includes('rate limit')){
    return "Email rate limit exceeded. Wait a bit, or use Google sign-in (no email needed).";
  }
