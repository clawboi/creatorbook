...
async function ensureProfile(){
  ...
  if(!prof){
    ...
    const ins = {
      ...
      // Always start un-onboarded so the Artist/Creator chooser shows on first login.
      onboarded: false,
...
async function signInWithPassword(){
  ...
  // NOTE: Supabase password auth uses EMAIL (or phone). Username sign-in requires
  // a server-side lookup (RPC/edge function). For now we accept "email or username"
  // in the UI, but we only sign in with email.
...
async function signUpWithPassword(){
  ...
  // Store any extra data locally (optional). Role is chosen after login.
