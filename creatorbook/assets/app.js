...
function hasAuthHash(){
  const p = hashParams();
  return p.has("access_token") || p.has("refresh_token") || p.has("error") || p.has("error_code") || p.get("type")==="recovery";
}

function clearAuthHash(to="home"){
  // Use replaceState so we don't create extra back-button steps.
  const base = location.pathname + location.search;
  history.replaceState(null, "", base + "#" + (to || "home"));
}

function prettyAuthError(){
  const p = hashParams();
  const code = p.get("error_code") || "";
  const desc = p.get("error_description") || p.get("error") || "";
  const msg = decodeURIComponent(desc.replace(/\+/g, " ")).trim();
  if(code === "otp_expired") return "That email link expired. Request a fresh one (Magic Link or Forgot Password).";
...
async function init(){
  initSupabase();
  // If Supabase redirected back with an error in the hash, show it and clean up.
  if(hasAuthHash()){
    const p = hashParams();
    if(p.has("error") || p.has("error_code")){
      const msg = prettyAuthError();
      if(msg) toast(msg);
      // If the link was bad/expired, keep the app usable by clearing the hash.
      if(p.get("error_code")==="otp_expired" || p.get("error")==="access_denied"){
        clearAuthHash("home");
      }
    }
  }
  ...
  if(APP.sb){
    APP.sb.auth.onAuthStateChange(async ()=>{
      ...
      if(APP.user){
        ...
        setAuthedUI();
        await refreshWallet();
        // Clean up any auth tokens / recovery flags in the URL so the app doesn't get stuck.
        if(hasAuthHash()) clearAuthHash(getHash() || 'home');
  ...
  on('authClose','click',()=>{ if(hasAuthHash()) clearAuthHash('home'); closeModal('authModal'); });
