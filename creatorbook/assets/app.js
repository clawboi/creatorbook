...
async function ensureProfile(){
  ...
  if(!prof){
    ...
    if(r.error && String(r.error.message||'').toLowerCase().includes('username')){
      toast('Username already taken. Open Profile → Settings and pick a different one.');
      // retry without username so the account can still proceed
      const r2 = await APP.sb.from('profiles').insert({ ...ins, username: null }).select('*').single();
      prof = r2.data || null;
    }

    prof = r.data || null;
  } else {
    // If username was set during signup but profile existed without it, patch it in.
    const pending = getPendingSignup(APP.user.email) || {};
    if(pending.username && !prof.username){
      const u = await APP.sb.from('profiles').update({ username: pending.username }).eq('id', APP.user.id).select('*').single();
      if(!u.error && u.data) prof = u.data;
    }
  }

  if(APP.rolePicked && prof.role !== APP.rolePicked){
    // update role if user picked during onboarding
    const r = await APP.sb.from('profiles').update({ role: APP.rolePicked, onboarded: true }).eq('id', APP.user.id).select('*').single();
    prof = r.data || prof;
  }
  APP.me = prof;
...
async function maybeOnboard(){
  ...
  // If they already completed onboarding, do nothing.
...
async function completeOnboarding(role){
  if(!APP.sb || !APP.user) return;
  const unameRaw = (byId('setUsername2')?.value || '').trim();
  if(unameRaw && !/^[a-zA-Z0-9_\.]{3,20}$/.test(unameRaw)) return toast('Username: 3–20 chars (letters, numbers, _ or .)');
...
function setAuthMode(mode){
  ...
  // Show only the relevant primary action button
  if(loginBtn) loginBtn.style.display = (APP.authMode==='login') ? '' : 'none';
  if(signupBtn) signupBtn.style.display = (APP.authMode==='signup') ? '' : 'none';


  // Role chooser removed (we onboard after login)
  const rw = byId("authRoleWrap");
  if(rw) rw.style.display = "none";

  // Username only for signup
  const uw = byId('authUsernameWrap');
  if(uw) uw.style.display = (APP.authMode==="signup") ? "" : "none";
  ...
  if(ph) ph.style.display = (APP.authMode==="signup") ? "" : "none";
...
function normalizeAuthError(err){
  ...
  if(low.includes('rate limit')){
    return "Email rate limit exceeded (Supabase is throttling emails). Wait a few minutes, then try again, or use Google sign-in.";
  }
...
async function signInMagicLink(){
  ...
  if(APP.authMode==="signup"){
    APP.rolePicked = (byId("authRole")?.value || "client");
    const em = (byId("authEmail")?.value || "").trim();
    if(em) setPendingRole(em, APP.rolePicked);
  }
  const email = (byId("authEmail")?.value || "").trim();
  const username = (byId('authUsername')?.value || '').trim();
  if(!email || !email.includes("@")) return toast("Enter a valid email");
  if(!username) return toast('Pick a username');
  if(!/^[a-zA-Z0-9_\.]{3,20}$/.test(username)) return toast('Username: 3–20 chars (letters, numbers, _ or .)');
...
async function signInWithGoogle(){
  if(!APP.sb) return toast("Supabase not configured");
  // If user is onboarding (Create account), remember chosen role for after OAuth.
  if(APP.authMode==="signup"){
    APP.rolePicked = (byId("authRole")?.value || "client");
    const em = (byId("authEmail")?.value || "").trim();
    if(em) setPendingRole(em, APP.rolePicked);
...
async function signInWithPassword(){
  ...
  // Capture role selection during signup
  APP.rolePicked = (byId("authRole")?.value || "client");
  setPendingRole(email, APP.rolePicked);
...
async function signUpWithPassword(){
  ...
  // Store onboarding data so after confirmation we can finish setup.
  APP.rolePicked = (byId("authRole")?.value || "client");
  setPendingRole(email, APP.rolePicked);
  setPendingSignup(email, { phone, username: username.toLowerCase() });
  const redirectTo = authRedirectTo();
  ...
  if(error) return toast(normalizeAuthError(error));
  if(!data?.session){
    toast("Account created. Check your email to confirm, then you’ll be signed in here.");
    setAuthMode("login");
...
function setHomeTab(which){
  const postsBtn = byId('homeTabPosts');
  const creatorsBtn = byId('homeTabCreators');
  const vPosts = byId('homeViewPosts');
  const vCreators = byId('homeViewCreators');
  const showPosts = (which !== 'creators');
  if(postsBtn) postsBtn.classList.toggle('active', showPosts);
  if(creatorsBtn) creatorsBtn.classList.toggle('active', !showPosts);
  if(vPosts) vPosts.style.display = showPosts ? '' : 'none';
  if(vCreators) vCreators.style.display = showPosts ? 'none' : '';
}
...
async function fetchPublicPosts(sort='recent'){
  ...
  if(sort === 'popular') q.order('likes_count', { ascending:false }).order('created_at', { ascending:false });
  else q.order('created_at', { ascending:false });
...
async function renderPublicPosts(){
  ...
  out.innerHTML = rows.map(p=>`
    <div class="card mini">
      <div class="cardTitle" style="margin-bottom:6px;">${esc(p.display_name || 'User')} <span class="muted">· ${esc(p.city||'Los Angeles')}</span></div>
      ${p.title ? `<div style="font-weight:700">${esc(p.title)}</div>` : ''}
      <div class="muted">${fmtDateTime(p.created_at)}${Number(p.likes_count||0) ? ` · ❤ ${Number(p.likes_count||0)}` : ''}</div>
...
async function renderProfile(){
  ...
  if(byId('setUsername2')) byId('setUsername2').value = m.username || '';
...
async function saveSettingsInline(){
  ...
  const patch = {
    ...
    username: ((byId('setUsername2')?.value || '').trim() || null)?.toLowerCase?.() || null,
...
// Home filters
  ...
  // Home tabs (Community vs Creators)
  on('homeTabPosts','click', ()=>setHomeTab('posts'));
  on('homeTabCreators','click', ()=>setHomeTab('creators'));
  ...
  on('btnSaveSettings','click', saveSettingsInline);
