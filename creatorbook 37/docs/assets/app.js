...
async function safeRoute(){
  ...
  try{
    ...
    await Promise.race([
      route(),
      sleep(20000).then(()=>{ throw new Error('route_timeout'); })
    ]);
  }catch(e){
...
function initSupabase(){
  ...
  APP.sb = window.supabase.createClient(c.url, c.anon, {
    auth: {
      ...
      // Reduce cross-tab lock pain when switching browsers/tabs.
      // If another tab is holding the auth-token lock, we wait longer before giving up.
...
async function refreshSession(){
  ...
  try{
    const { data } = await Promise.race([
      ...
      sleep(8000).then(()=>({ data: { session: APP.session } }))
    ]);
...
async function init(){
  ...
  if(APP.sb){
    APP.sb.auth.onAuthStateChange((_event, session)=>{
      // IMPORTANT: keep this handler synchronous.
      // Supabase documents a deadlock bug if you await async Supabase calls inside onAuthStateChange.
      // We'll just update in-memory state and schedule rendering work outside this callback.
      if(APP._booting) return;

      APP.session = session || null;
      APP.user = session?.user || null;
      if(!APP.user){ APP.me = null; APP.wallet = null; }

      try{ if(authArtifactsPresent()) clearAuthArtifacts(getHash()); }catch(_e){}

      // Render/update outside the auth callback (prevents auth-token lock deadlocks).
      scheduleRoute(0);
    });
  }
