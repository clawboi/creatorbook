...
function setAuthedUI(){
  ...
  const homePublicCtas = byId("homePublicCtas");
  if(homePublicCtas) homePublicCtas.style.display = authed ? "none" : "";
...
// Home filters
  ...
  if(byId('homeCtaCreate')) byId('homeCtaCreate').addEventListener('click', goCreate);
  if(byId('homeCtaSignIn')) byId('homeCtaSignIn').addEventListener('click', ()=>{
    openModal('authModal');
    setAuthMode('login');
  });
