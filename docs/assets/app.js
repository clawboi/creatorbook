...
async function safeRoute(){
  ...
  _routeInFlight = true;
  try{
    // IMPORTANT: route(), not safeRoute() (avoids recursion + "dead clicks")
    await route();
  }catch(e){
...
async function init(){
  ...
  const recoverUI = ()=>{
// cbPointerRecover: if an invisible full-screen fixed layer is eating clicks, nuke it on first interaction.
const cbPointerRecover = (evt)=>{
  try{
    const vw = window.innerWidth, vh = window.innerHeight;
    const el = document.elementFromPoint(Math.max(1, vw/2), Math.max(1, vh/2));
    if(!el) return;
    const cs = getComputedStyle(el);
    const z = parseInt(cs.zIndex || "0", 10) || 0;
    const fixed = (cs.position === "fixed");
    const blocks = fixed && cs.pointerEvents !== "none" && z >= 40;
    if(blocks){
      killClickShields();
      Promise.resolve().then(()=>safeRoute());
    }
  ...
  window.addEventListener("visibilitychange", ()=>{ if(!document.hidden){ recoverUI(); setTimeout(recoverUI, 120); } });
  window.addEventListener("focus", ()=>{ recoverUI(); setTimeout(recoverUI, 120); });
  window.addEventListener("pageshow", (e)=>{ recoverUI(); try{ if(e && e.persisted) setTimeout(recoverUI, 120); }catch(_e){} });
document.addEventListener("pointerdown", cbPointerRecover, { capture:true, passive:true });
