...
function bindNavDelegate(){
  ...
  document.addEventListener('click', (e)=>{
    ...
    if(!a) return;
    // let cmd/ctrl clicks open new tabs normally
...
async function init(){
  ...
  const recoverUI = ()=>{
const cbPointerRecover = ()=>{
  try{
    const vw = window.innerWidth, vh = window.innerHeight;
    const top = document.elementFromPoint(Math.max(1, vw/2), Math.max(1, vh/2));
    if(!top) return;
    const cs = getComputedStyle(top);
    const z = parseInt(cs.zIndex || "0", 10) || 0;
    if(cs.position === "fixed" && cs.pointerEvents !== "none" && z >= 40){
      killClickShields();
      scheduleRoute(0);
    }
  }catch(_e){}
