...
// cbPointerRecover: declared globally so tab-restore code never throws.
...
async function safeRoute(){
  ...
  _routeInFlight = true;
  try{ await route(); }
  catch(e){ console.error('route failed', e); try{ toast('Recovering view…'); }catch(_e){} }
  finally{
    _routeInFlight = false;
    if(_routeQueued){ _routeQueued = false; Promise.resolve().then(()=>safeRoute()); }
  }
}
...
// CB_NAV_DELEGATE: one capture listener for nav links so switching tabs/bfcache
// never results in "can't navigate until refresh".
