...
async function authLogin(){
  ...
  // Use returned session immediately (avoids race where getSession() is still null)
  APP.session = data?.session || null;
  APP.user = data?.user || data?.session?.user || null;

  if(!APP.user){
    // Fallback once (in case of very slow storage/session write)
  ...
  if(!APP.user){
    return toast('Signed in but session missing. Check browser storage + Supabase Auth URL settings.');
  }
...
async function authSignup(){
  ...
  // If confirmations are OFF, Supabase returns a session right away.
  APP.session = data?.session || null;
  APP.user = data?.user || data?.session?.user || null;
  ...
  if(APP.user){
    ...
    toast('Account created. Check your email if confirmation is on, then sign in.');
    setAuthMode('login');
  }
}
