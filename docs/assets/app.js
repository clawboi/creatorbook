...
async function uploadToBucket(bucket, file, folder){
  if(!APP.sb) throw new Error("Supabase not ready");
  const ext = (file.name && file.name.includes('.')) ? file.name.split('.').pop() : 'bin';
  const safeExt = ext.toLowerCase().replace(/[^a-z0-9]/g,'') || 'bin';
  const fname = `${Date.now()}_${Math.random().toString(16).slice(2)}.${safeExt}`;
  const path = `${folder}/${APP.user.id}/${fname}`;
  ...
  if(!url) throw new Error("No public URL (bucket must be public)");
  return url;
}

async function uploadAvatarFromInput(){
  const avIn = byId('avatarFile') || byId('topAvatarFile');
  const file = avIn?.files?.[0];
  if(!file) return;
  if(!APP.user) return toast('Sign in first');
  try{
    const url = await uploadToBucket('avatars', file, 'profile');
    const r = await APP.sb.from('profiles').update({ avatar_url:url }).eq('id', APP.user.id);
    if(r.error){
      console.warn(r.error);
      localStorage.setItem('CB_AVATAR_URL', url);
      toast('Avatar saved locally (DB column missing?)');
    }else{
    ...
    console.warn(e);
    toast('Avatar upload needs Storage bucket "avatars" (public).');
  }
}
...
async function renderPost(){
  ...
  if(!APP._bindHomeCtas){
    ...
    const frBtn = byId('feedModeFriends');
    const setMode = async (m)=>{
      APP.feedMode = m;
      if(allBtn) allBtn.classList.toggle('active', m==='all');
      if(frBtn) frBtn.classList.toggle('active', m==='friends');
      await renderPublicPosts();
    };
    if(allBtn) allBtn.addEventListener('click', ()=>setMode('all'));
    if(frBtn) frBtn.addEventListener('click', ()=>setMode('friends'));
...
async function renderBook(){
  ...
  const renderServices = ()=>{
    ...
    let list = (BOOK_MENU[cat] || []).slice();
  if(!list.includes('Other')) list.push('Other').filter(x=>x!=='Custom Request');
...
async function fetchPublicPosts(){
  ...
  try{
    ...
    if(APP.feedMode === 'friends' && APP.user){
...
async function openUserProfile(userId){
  ...
  const pr = await APP.sb.from('profiles').select('id,display_name,city,avatar_url,bio,role_primary,role_secondary,portfolio_url').eq('id', userId).single();
  if(pr.error){ console.warn(pr.error); if(header) header.textContent='Could not load profile.'; return; }
  const p = pr.data;
...
async function bindPackageMedia(){
  APP.pkgMedia = APP.pkgMedia || { imageUrls:[], videoUrl:'' };
  const imgIn = byId('pkgMediaImages');
  const vidIn = byId('pkgMediaVideo');
  const prev = byId('pkgMediaPreview');

  const renderPrev = ()=>{
    if(!prev) return;
    const imgs = (APP.pkgMedia.imageUrls||[]).map(u=>`<img src="${esc(u)}" style="width:84px; height:84px; object-fit:cover; border-radius:14px; border:1px solid rgba(255,255,255,.12);" />`).join('');
    const vid = APP.pkgMedia.videoUrl ? `<video src="${esc(APP.pkgMedia.videoUrl)}" controls style="width:100%; max-height:240px; border-radius:16px; margin-top:10px;"></video>` : '';
    prev.innerHTML = `<div class="row" style="gap:10px; flex-wrap:wrap;">${imgs||'<div class="muted">No media yet</div>'}</div>${vid}`;
  };

  const uploadImages = async ()=>{
    if(!imgIn?.files?.length) return;
    if(!APP.user) return toast('Sign in first');
    try{
      const files = Array.from(imgIn.files).slice(0,6);
      APP.pkgMedia.imageUrls = [];
      renderPrev();
      for(const f of files){
        const url = await uploadToBucket('media', f, 'packages');
        APP.pkgMedia.imageUrls.push(url);
        renderPrev();
      }
      toast('Images uploaded');
    }catch(e){
      console.warn(e);
      toast('Image upload needs Storage bucket "media" (public).');
    }
  };

  const uploadVideo = async ()=>{
    const f = vidIn?.files?.[0];
    if(!f) return;
    if(!APP.user) return toast('Sign in first');
    try{
      const url = await uploadToBucket('media', f, 'packages');
      APP.pkgMedia.videoUrl = url;
      renderPrev();
      toast('Video uploaded');
    }catch(e){
      console.warn(e);
      toast('Video upload needs Storage bucket "media" (public).');
    }
  };

  if(imgIn && !imgIn._bound){
    imgIn._bound = true;
    imgIn.addEventListener('change', ()=>uploadImages());
  }
  if(vidIn && !vidIn._bound){
    vidIn._bound = true;
    vidIn.addEventListener('change', ()=>uploadVideo());
  }
  renderPrev();
}
