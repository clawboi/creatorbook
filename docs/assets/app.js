...
async function renderPost(){
  ...
  if(!APP._bindHomeCtas){
    ...
    if(cBook) cBook.addEventListener('click', ()=>{
      location.hash = '#book';
    });
  }
}
...
async function renderBook(){
  ...
  // Bind recommender UI once
  if(!APP._bindBookUI){
    APP._bindBookUI = true;

    const budget = byId('bookBudget');
    const budgetLabel = byId('bookBudgetLabel');
    const serviceSel = byId('bookService');
    const tierPref = byId('bookTierPref');
    const sortSel = byId('bookSort');
    const btnRec = byId('btnRecommend');

    const syncBudget = ()=>{
      const v = Number(budget?.value||0);
      if(budgetLabel) budgetLabel.textContent = String(v) + ' credits';
    };
    if(budget) budget.addEventListener('input', syncBudget);
    syncBudget();

    const rerender = async ()=>{ await renderBookFeed(); };

    if(serviceSel) serviceSel.addEventListener('change', rerender);
    if(tierPref) tierPref.addEventListener('change', rerender);
    if(sortSel) sortSel.addEventListener('change', rerender);
    if(btnRec) btnRec.addEventListener('click', async ()=>{
      // "Recommend" = same render but with a little nudge: snap to nearest tier if user picked none
...
async function renderBookFeed(opts={}){
  ...
  const budget = Number(byId('bookBudget')?.value||0);
  const serviceNeed = (byId('bookService')?.value||'').trim();
  let tierPref = (byId('bookTierPref')?.value||'').trim().toLowerCase();
  ...
  // normalize tier
  const tierRank = { bronze:1, silver:2, gold:3, elite:4 };
  const rankToTier = { 1:'bronze', 2:'silver', 3:'gold', 4:'elite' };

  // Optional: if Recommend clicked and no preference, pick tier based on budget buckets
  if(opts.recommend && !tierPref){
    if(budget <= 250) tierPref = 'bronze';
    else if(budget <= 600) tierPref = 'silver';
    else if(budget <= 1100) tierPref = 'gold';
    else tierPref = 'elite';
    const sel = byId('bookTierPref');
    if(sel) sel.value = tierPref;
  }

  // filter by service
  let filtered = all.slice();
  if(serviceNeed){
    filtered = filtered.filter(p => String(p.service||'').toLowerCase() === serviceNeed.toLowerCase());
  }

  // group by title/service (creator packages have 1..4 tier rows)
  ...
  // choose best tier within budget for each group
  const picks = [];
  for(const [k, items] of byGroup.entries()){
    const sortedTiers = items.slice().sort((a,b)=>{
      const ra = tierRank[String(a.tier||'').toLowerCase()] || 999;
      const rb = tierRank[String(b.tier||'').toLowerCase()] || 999;
      return ra - rb;
    });

    // enforce tier preference if chosen
    let candidates = sortedTiers;
    if(tierPref){
      candidates = sortedTiers.filter(x => String(x.tier||'').toLowerCase() === tierPref);
      if(!candidates.length) candidates = sortedTiers; // fallback
    }

    // within budget
    let within = candidates.filter(x => Number(x.price_credits||0) <= budget);
    if(!within.length){
      // allow slightly above budget: up to +15% so recs don't go empty too fast
      within = candidates.filter(x => Number(x.price_credits||0) <= Math.ceil(budget*1.15));
    }
    if(!within.length) continue;

    // prefer closest to budget (higher value) but not crazy
    within.sort((a,b)=> Number(b.price_credits||0) - Number(a.price_credits||0));
    ...
    // score: closeness to budget + tier alignment
    const price = Number(chosen.price_credits||0);
    const closeness = budget ? (1 - Math.min(1, Math.abs(budget - price)/Math.max(1,budget))) : 0;
    const t = String(chosen.tier||'').toLowerCase();
    const prefBonus = tierPref && t===tierPref ? 0.25 : 0;

    const score = closeness + prefBonus;

    picks.push({ chosen, items:sortedTiers, name, city, score, created_at: chosen.created_at });
  }

  // sort
  if(sort === 'new'){
    picks.sort((a,b)=> String(b.created_at||'').localeCompare(String(a.created_at||'')));
  }else if(sort === 'low'){
    picks.sort((a,b)=> Number(a.chosen.price_credits||0) - Number(b.chosen.price_credits||0));
  }else if(sort === 'high'){
    picks.sort((a,b)=> Number(b.chosen.price_credits||0) - Number(a.chosen.price_credits||0));
  }else{
    picks.sort((a,b)=> (b.score - a.score) || (Number(b.chosen.price_credits||0) - Number(a.chosen.price_credits||0)));
  }

  if(metaEl){
    const svc = serviceNeed ? serviceNeed : 'all services';
    metaEl.textContent = `Showing ${picks.length} matches · ${svc} · budget ${budget} credits`;
  }
  ...
  // render cards with all tiers, highlight recommended
  feedEl.innerHTML = picks.map(p=>{
    const first = p.chosen;
    const profLabel = `${p.name}`;
    const tiers = p.items.map(tier=>{
      const t = String(tier.tier||'').toLowerCase();
      const isPick = String(tier.id) === String(first.id);
      return `
        <div class="card mini" style="margin-top:10px; border-color:${isPick ? 'rgba(138,46,255,.45)' : 'rgba(255,255,255,.12)'};">
          <div class="row" style="justify-content:space-between; align-items:flex-start;">
            <div>
              <div class="pill">${esc(String(t||'').toUpperCase())}${isPick ? ' · RECOMMENDED' : ''}</div>
              <div class="muted" style="margin-top:6px;">${esc(tier.delivery_days != null ? (tier.delivery_days + ' days') : '')}</div>
              ${tier.includes ? `<div style="margin-top:6px; white-space:pre-wrap">${esc(tier.includes)}</div>` : ``}
              ${tier.addons ? `<div class="muted" style="margin-top:6px; white-space:pre-wrap">Add-ons: ${esc(tier.addons)}</div>` : ``}
            </div>
            <div style="text-align:right;">
              <div class="cardTitle" style="margin:0;">${moneyCredits(tier.price_credits||0)}</div>
              <div class="muted">credits</div>
              ${APP.user ? `<button class="btn ${isPick ? '' : 'ghost'}" data-bookpkg="${esc(tier.id)}" style="margin-top:8px;" type="button">${isPick ? 'Book now' : 'Book'}</button>` : `<div class="muted" style="margin-top:8px;">Sign in to book</div>`}
            </div>
          </div>
        </div>
      `;
      ...
      <div class="card">
        <div class="row" style="justify-content:space-between; align-items:flex-start;">
          <div>
            ...
            <div class="muted">${esc(first.service||'')} · Sold by <b>${esc(profLabel)}</b> · ${esc(p.city)}</div>
          </div>
        </div>
        ${tiers}
      </div>
  ...
  $$('button[data-bookpkg]', feedEl).forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      ...
      const pick = picks.flatMap(x=>x.items).find(x=>String(x.id)===String(pkgId));
      if(!pick) return;
      // profile lookup
