...
/* ----------------------------- focus recovery ----------------------------- */
// When you switch browser tabs and come back, Chrome/Safari can sometimes leave a modal/menu “ghost layer”
// or stale event state. We aggressively reset overlays and re-render the current route on focus.
function closeAllOverlays(){
  try{
    // close any open modals
    $$('.modal').forEach(m=>{ try{ m.style.display = 'none'; }catch(_e){} });
    // close profile menu dropdown
    const pm = byId('profileMenu');
    if(pm) pm.style.display = 'none';
    // remove any accidental click-blocking styles
...
document.addEventListener('visibilitychange', ()=>{
  if(!document.hidden){
    try{ closeAllOverlays(); }catch(_e){}
    // re-bind nav + refresh view
