...
async function safeRoute(){
  try{
    ...
    try{ toast('Recovering view…'); }catch(_){}
  }
}
...
function openModal(id){ const m = byId(id); if(!m) return; m.classList.add("open"); m.style.display="grid"; }
function closeModal(id){ const m = byId(id); if(!m) return; m.classList.remove("open"); m.style.display="none"; }
...
async function init(){
  ...
  const killClickShields = ()=>{
    try{
      ...
      // hard-kill ANY modal overlay that might be stuck
      document.querySelectorAll(".modal").forEach(m=>{ if(!m.classList.contains("open")){ m.classList.remove("open"); m.style.display="none"; m.style.pointerEvents="none"; } });
