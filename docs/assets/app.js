...
function setAuthedUI(){
  ...
  const pill = byId("creditsPill");
  if(pill && authed) pill.textContent = bal;
}
...
async function fetchHomeFeed(){
  ...
  // creator_public view might exist; fail softly
...
async function renderHome(){
  ...
  // bind view buttons (NO btn.add!)
  $$('button[data-view]', feedEl).forEach(btn=>{
    btn.addEventListener('click', async (e)=>{
      e.preventDefault();
      const id = btn.getAttribute('data-view');
      if(id) await openCreatorProfile(id);
    });
  });
