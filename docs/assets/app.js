...
/* ----------------------------- click-lock recovery ----------------------------- */
/*
  Some browsers (and some UI states) can leave an invisible full-screen layer "on top"
  after tab switching, making the app feel frozen until refresh.
  We aggressively clear anything that could be intercepting pointer events whenever
  the tab becomes visible again.
*/
function hardRecoverUI(reason){
  try{
    // Close any modals/sheets/backdrops
    document.querySelectorAll('.modal').forEach(m=>{ m.style.display = 'none'; });
    // Close profile menu dropdown
    const pm = byId('profileMenu');
    if(pm) pm.style.display = 'none';

    // Undo any accidental scroll/pointer locks
    document.body.style.overflow = '';
    document.documentElement.style.overflow = '';
    document.body.style.pointerEvents = '';
    document.documentElement.style.pointerEvents = '';

    // Re-render current route (hash may have changed while the UI was "stuck")
    try{ route(); }catch(_e){}
  }catch(_e){}
}

// Recover when returning to the tab/window (Chrome/Safari can suspend timers/events)
window.addEventListener('focus', ()=>hardRecoverUI('focus'));
window.addEventListener('pageshow', ()=>hardRecoverUI('pageshow'));
document.addEventListener('visibilitychange', ()=>{
  if(!document.hidden) hardRecoverUI('visibility');
});

// If something throws in the background, auto-recover instead of staying unclickable
window.addEventListener('error', ()=>hardRecoverUI('error'));
window.addEventListener('unhandledrejection', ()=>hardRecoverUI('unhandledrejection'));
...
// nav links: route immediately (even if hash doesn't change)
try{
  $$('nav.nav a[data-nav]').forEach(a=>{
    if(a._bound) return;
    a._bound = true;
    a.addEventListener('click', (e)=>{
      ...
      if(location.hash === href){
        route(); // same hash won't trigger hashchange
      }else{
        location.hash = href;
      }
    });
